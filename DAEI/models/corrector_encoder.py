...
class CorrectorEncoderModel(transformers.PreTrainedModel):
    ...
    def __init__(
        ...
        # DAE Shield: optional frozen DAE for denoising embeddings before the
        # diff/transform pipeline.  Injected via set_dae() after construction.
        self.dae: Optional[nn.Module] = None
        self.use_dae_shield: bool = False
        self._dae_shield_call_verified: bool = False
        self._dae_noise_sigma: float = 0.01

    def set_dae(self, dae_model: nn.Module, noise_sigma: float = 0.01) -> None:
        """Inject a frozen DAE for the DAE Shield pipeline."""
        self.dae = dae_model
        self.use_dae_shield = True
        self._dae_noise_sigma = noise_sigma
    ...
    def get_encoder_embedding(
        ...
        if self.use_dae_shield and self.dae is not None:
            with torch.no_grad():
                _sigma_e = torch.full(
                    (embedding.shape[0],), self._dae_noise_sigma,
                    device=embedding.device, dtype=embedding.dtype,
                )
                _sigma_h = torch.full(
                    (hypothesis_embedding.shape[0],), self._dae_noise_sigma,
                    device=hypothesis_embedding.device, dtype=hypothesis_embedding.dtype,
                )
                if not self._dae_shield_call_verified:
                    self._dae_shield_call_verified = True
                    orig = embedding.detach().clone()
                    embedding = self.dae(embedding, sigma=_sigma_e)
                    hypothesis_embedding = self.dae(hypothesis_embedding, sigma=_sigma_h)
                    l2 = (embedding - orig).norm(dim=1).mean().item()
                    print(
                        "[DAE Shield verification] DAE was called during training | "
                        f"L2(denoised-noisy)={l2:.6f} (nonzero confirms denoising)"
                    )
                else:
                    embedding = self.dae(embedding, sigma=_sigma_e)
                    hypothesis_embedding = self.dae(hypothesis_embedding, sigma=_sigma_h)
