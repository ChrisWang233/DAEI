...
from DAEI.models.model_utils import (
    FREEZE_STRATEGIES,
    disable_dropout,
    freeze_params,
...
from DAEI.utils import embed_api
...
class InversionModel(transformers.PreTrainedModel):
    ...
    def __init__(self, config: InversionConfig):
        ...
        self.embedder_model_name = config.embedder_model_name
    ...
    def _process_embedder_output(
        self,
        outputs: Union[torch.Tensor, transformers.modeling_outputs.BaseModelOutput],
        attention_mask: torch.Tensor,
    ) -> torch.Tensor:
        # MockEmbedder / some compiled or return_dict=False paths return (B, T, D) hidden states.
        if isinstance(outputs, torch.Tensor):
            embeddings = mean_pool(outputs, attention_mask)
        elif self.embedder_model_name == "gte_base":
            # GTE sentence embeddings use mean pooling + L2 normalization.  The
            # raw AutoModel also exposes BERT's pooler_output, but that is not
            # the sentence embedding and collapses unrelated examples together.
