...
def tensorize_token_id_sequences(
    ...
    *,
    ...
    padded = [
        list(sequence) + [pad_token_id] * (max_length - len(sequence))
        for sequence in sequences
    ]
    return torch.tensor(padded, dtype=torch.long, device=device)


def metric_for_best_model_candidate_keys(
    metric_for_best_model: Optional[str],
) -> List[str]:
    """Keys to look up in ``metrics`` for ``TrainingArguments.metric_for_best_model``."""
    if not metric_for_best_model:
        return []
    keys = [metric_for_best_model]
    if not str(metric_for_best_model).startswith("eval_"):
        keys.append(f"eval_{metric_for_best_model}")
    return keys


def filter_eval_metrics_for_log(
    metrics: Dict,
    metric_for_best_model: Optional[str] = None,
) -> Dict:
    """Keep only the compact eval metrics used for experiment tracking.

    If *metric_for_best_model* is set (e.g. ``eval_nq,msmarco_loss``), that key is
    always preserved so ``Trainer._save_checkpoint`` / ``load_best_model_at_end``
    still find the metric when ``eval_log_text_metrics_only`` is True.
    """
    endings = (
        "_loss",
        "bleu_score",
    ...
    def _keep(key: str) -> bool:
        if key == "epoch":
            return True
        return any(key.endswith(suffix) for suffix in endings)

    out = {k: v for k, v in metrics.items() if _keep(k)}
    for k in metric_for_best_model_candidate_keys(metric_for_best_model):
        if k in metrics:
            out[k] = metrics[k]
            break
    return out
...
class BaseTrainer(transformers.Trainer):
    ...
    def train(self, resume_from_checkpoint=None, trial=None, **kwargs):
        """Override so that when resuming from checkpoint we use current args' batch size,
        not the checkpoint's train_batch_size (which may be from a different GPU setup).
        """
        if resume_from_checkpoint is not None:
            from transformers.trainer import TrainerState, TRAINER_STATE_NAME
            state_path = os.path.join(resume_from_checkpoint, TRAINER_STATE_NAME)
            if os.path.isfile(state_path):
                _orig_load = TrainerState.load_from_json

                def _load_use_current_batch_size(path):
                    state = _orig_load(path)
                    state.train_batch_size = self.args.train_batch_size
                    return state

                TrainerState.load_from_json = _load_use_current_batch_size
                try:
                    return super().train(
                    ...
                    TrainerState.load_from_json = _orig_load
        return super().train(
            resume_from_checkpoint=resume_from_checkpoint,
            trial=trial,
            **kwargs,
    ...
    def compare_trainer_and_checkpoint_args(self, training_args, trainer_state):
        """Log mismatches like the parent, then sync step intervals from current args.

        HuggingFace's ``DefaultFlowCallback`` uses ``trainer_state.eval_steps`` /
        ``logging_steps`` / ``save_steps`` (from ``trainer_state.json``), not
        ``training_args.*``. After resume, those stay at the old run's values even
        if you pass new ``--eval_steps`` on the CLI. Overwrite with current args
        so resume respects updated flags.
        """
        super().compare_trainer_and_checkpoint_args(training_args, trainer_state)
        synced = []
        for attr in ("logging_steps", "eval_steps", "save_steps"):
            v = getattr(training_args, attr, None)
            if v is None:
                continue
            if isinstance(v, (int, float)) and v >= 1:
                new_v = int(v)
                old_v = getattr(trainer_state, attr, None)
                if old_v != new_v:
                    synced.append(f"{attr}: {old_v} -> {new_v}")
                setattr(trainer_state, attr, new_v)
        if synced:
            logger.info(
                "Resume: TrainerState step intervals synced from current args (%s). "
                "(HF callbacks use state.*, not args.*.)",
                "; ".join(synced),
            )
    ...
    def enable_emb_cos_sim_metric(self) -> None:
        self.additional_metrics.append(DAEI.metrics.EmbeddingCosineSimilarity())
    ...
    def _load_rng_state(self, checkpoint):
        """Load RNG state from checkpoint. Uses weights_only=False for torch.load
        so that RNG files (which contain numpy/python state) work under PyTorch 2.6+.
        """
        if checkpoint is None:
            return
        _orig_load = torch.load
        try:
            def _load_rng(*args, **kwargs):
                kwargs["weights_only"] = False
                return _orig_load(*args, **kwargs)
            torch.load = _load_rng
    ...
    def eval_generation_metrics(
        ...
        preds_sample = tensorize_token_id_sequences(
            preds_sample_list,
            pad_token_id=self.pad_token_id,
            device=self.args.device,
        )[:128]
        preds_sample_labels = tensorize_token_id_sequences(
    ...
    def _broadcast_metric_for_best_model_if_missing(self, metrics: Dict) -> None:
        """Copy the best-model metric from rank 0 so every rank has it for checkpointing.

        Non-zero ranks can end up with only throughput keys when generation metrics are
        gated on ``local_rank``; ``Trainer._save_checkpoint`` still runs on all ranks.
        """
        if not torch.distributed.is_available() or not torch.distributed.is_initialized():
            return
        if torch.distributed.get_world_size() <= 1:
            return
        m = getattr(self.args, "metric_for_best_model", None)
        if not m:
            return
        resolved_key = None
        for k in metric_for_best_model_candidate_keys(m):
            if k in metrics:
                resolved_key = k
                break
        ...
        if rank == 0:
            obj_list = (
                [resolved_key, metrics[resolved_key]]
        ...
        if rank != 0 and obj_list[0] is not None and obj_list[1] is not None:
            metrics[obj_list[0]] = obj_list[1]
