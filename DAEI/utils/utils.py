...
def _rmtree_retry(path: str, attempts: int = 10, delay_s: float = 0.2) -> None:
    """Best-effort remove; retries help NFS / delayed handle release (Errno 39)."""
...
def dataset_map_multi_worker(
    ...
    """Distributed: shard dataset by world_size, map each shard, concatenate, delete shards.

    Failure mode that often appears only *after* a val-cache miss (e.g. new cache name or
    deleted ``*_val.arrow``): each rank runs ``Dataset.map(num_proc>1)``, then later
    ``shutil.rmtree`` on ``~/.cache/inversion/*_subshard_*.cache``.  Multiprocessing map
    can leave temp files; every rank also loads all shards via ``load_from_disk``, so mmap
    handles and NFS ``.nfs*`` stubs can keep directories "not empty" (Errno 39).

    Mitigations used below: under DDP force ``num_proc=1`` for the map (callers still pass
    ``num_proc`` for single-GPU behaviour), drop shard loader refs + ``gc`` before rmtree,
    and retry rmtree.  Non-distributed behaviour is unchanged.
    """
    try:
        rank = torch.distributed.get_rank()
        world_size = torch.distributed.get_world_size()
        # See docstring — multi-process map inside each rank makes rmtree flaky on shared FS.
        kwargs["num_proc"] = 1
    except (RuntimeError, ValueError):
    ...
    cache_path = (
        os.environ.get("DAEI_CACHE")
        or os.environ.get("VEC2TEXT_CACHE")
        or os.path.expanduser("~/.cache/inversion")
    )
    ...
    torch.distributed.barrier()
    shard_parts = [datasets.load_from_disk(p) for p in ds_shard_filepaths]
    full_dataset = datasets.concatenate_datasets(shard_parts)
    del shard_parts
    gc.collect()
    torch.distributed.barrier()
    print("rank", rank, "deleting:", ds_shard_filepaths[rank])
    _rmtree_retry(ds_shard_filepaths[rank])
    return full_dataset
...
class MockEmbedder:
    ...
    def eval(self):
        """Match ``nn.Module.eval`` so ``InversionModel.call_embedding_model`` can call it."""
        self.training = False
        return self
